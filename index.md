...
| database credential | `mppdb_ro`: SELECT on `dp2`, `ppdb`, `ssp` and `TAP_SCHEMA`, plus `system.one` and `system.parts`. Nothing more — the grant matches the served set exactly |
